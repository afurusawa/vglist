"use strict";

angular.module("app", [ "ngRoute" ]), angular.module("app").controller("gameController", [ "$scope", "$http", "$log", function(a, b, c) {
    c.log("game controller active"), a.userData = "", a.rating = 0, a.gameState = {
        added: !1,
        text: "add to list"
    };
    var d = !1;
    a.$watchGroup([ "init", "rating", "hoursPlayed" ], function(c, e) {
        if (c[1] > 0) {
            console.log("updating rating..." + a.rating);
            var f = {
                gameId: a.init.gameId,
                rating: a.rating
            };
            b.post("/updateGameRating", f).then(function(a) {
                console.log("updated rating");
            });
        }
        if (c[3] > 0) {
            console.log("updating hours played..." + a.hoursPlayed);
            var f = {
                gameId: a.init.gameId,
                hoursPlayed: a.hoursPlayed
            };
            b.post("/updateHoursPlayed", f).then(function(a) {
                console.log("updated hours played");
            });
        }
        c[0] && !d && b.get("/" + a.init.userId + "/" + a.init.gameId).then(function(b) {
            console.log("getting user info about the game"), d = !0, a.userData = b.data;
            var c = b.data;
            c && (a.gameState.added = !0, a.gameState.text = "added"), a.rating = c.rating, 
            a.rating > 0 && (a.rateButton.visible = !1);
        });
    }), a.rateButton = {
        visible: !0
    }, a.rateGame = function() {
        a.rateButton.visible = !a.rateButton.visible;
    }, a.addToList = function(c, d) {
        console.log("clicked add button w/ id: " + c.toString());
        var e = {
            gameId: c,
            gameName: d
        };
        b.post("/addToGameList", e).then(function(b) {
            console.log("res: " + JSON.stringify(b, null, 4)), a.gameState.added = !0, a.gameState.text = "added";
        });
    };
} ]), angular.module("app").controller("listController", [ "$scope", "$http", "$log", function(a, b, c) {
    c.log("list controller active");
    new Array();
    b.get("/allGames").then(function(b) {
        a.list = b.data;
    }), b.get("/allUsers").then(function(b) {
        a.userlist = b.data;
    }), a.button = "add to list", a.addToList = function(a, c) {
        console.log("clicked add button w/ id: " + a.toString());
        var d = {
            gameId: a,
            gameName: c
        };
        b.post("/addToGameList", d).then(function(a) {
            console.log("res: " + JSON.stringify(a, null, 4));
        });
    };
} ]), angular.module("app").controller("searchController", [ "$scope", "$http", "$log", function(a, b, c) {
    a.search = "", a.$watch("search", function() {
        a.search.length >= 3 && b.get("/search/" + a.search).then(function(b) {
            console.log("res: " + JSON.stringify(b, null, 4)), a.results = b.data;
        });
    });
} ]), angular.module("app").controller("submitGameController", [ "$scope", "$http", "$log", function(a, b, c) {
    a.platformData = [], a.consoles = [ {
        name: "Playstation 4",
        manufacturer: "Sony",
        state: !1
    }, {
        name: "Xbox One",
        manufacturer: "Microsoft",
        state: !1
    }, {
        name: "Wii U",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "PlayStation 3",
        manufacturer: "Sony",
        state: !1
    }, {
        name: "Wii",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "Xbox 360",
        manufacturer: "Microsoft",
        state: !1
    }, {
        name: "Xbox",
        manufacturer: "Microsoft",
        state: !1
    }, {
        name: "GameCube",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "PlayStation 2",
        manufacturer: "Sony",
        state: !1
    }, {
        name: "Dreamcast",
        manufacturer: "Sega",
        state: !1
    }, {
        name: "Nintendo 64",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "PlayStation",
        manufacturer: "Sony",
        state: !1
    } ], a.handhelds = [ {
        name: "PlayStation Vita",
        manufacturer: "Sony",
        state: !1
    }, {
        name: "Nintendo 3DS",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "PlayStation Portable",
        manufacturer: "Sony",
        state: !1
    }, {
        name: "Nintendo DS",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "Game Boy Advance",
        manufacturer: "Nintendo",
        state: !1
    }, {
        name: "Game Boy Color",
        manufacturer: "Nintendo",
        state: !1
    } ], a.toggle = function() {
        this.unit.state = !this.unit.state;
        for (var b = [], c = 0; c < a.consoles.length; c++) a.consoles[c].state && b.push(a.consoles[c].name);
        a.platformData = b;
    }, a.submitGame = function() {
        var c = {
            title: a.game.title,
            series: a.game.series,
            released: a.game.released,
            developer: a.game.developer,
            publisher: a.game.publisher,
            platform: a.platformData
        };
        console.log(JSON.stringify(c, null, 4));
        var d = {
            headers: {
                "Content-Type": "multipart/form-data"
            }
        };
        b.post("/addGame", c, d).then(function(a) {});
    };
} ]), angular.module("app").controller("headerController", [ "$scope", "$http", "$log", function(a, b, c) {
    a.login = {
        state: !1
    }, a.toggle = function() {
        a.login.state = !a.login.state;
    }, a.search = "", a.$watch("search", function() {
        a.search.length >= 3 && b.get("/search/" + a.search).then(function(b) {
            console.log("res: " + JSON.stringify(b, null, 4)), a.results = b.data;
        });
    });
} ]);